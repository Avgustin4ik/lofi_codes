...
public:
	...
	CamberLineFunction() {};
	~CamberLineFunction() {};
	CamberLineFunction(BezierCurve<T> &_curve, BezierCurve<T> &_assist_curve) :curve(_curve), assist_curve(_assist_curve), point(Vertex2D<T>()), t(0.5), betta(0.0), dt(0.0, 0.0), k1(curve.curvature(0.0)), k2(curve.curvature(1.0)) {};
	CamberLineFunction(BezierCurve<T> &_curve, BezierCurve<T> &_assist_curve, Vertex2D<T>& _point) :curve(_curve), assist_curve(_assist_curve), point(_point), t(0.5), betta(0.0), dt(0.0, 0.0), k1(curve.curvature(0.0)), k2(curve.curvature(1.0)) { };
	Vertex2D<T> getBezierPoint(const vector<T>& variables)
	{
		vector<Vertex2D<T>> &ppoints = curve.PPoints;
		vector<Vertex2D<T>> temp_ppoints = curve.PPoints;
	...
	{
		vector<T> variables(2);
		variables[0] = x[0];
		variables[1] = x[1];
		return coincidence_condition(variables) + tangent_condition();
	}
